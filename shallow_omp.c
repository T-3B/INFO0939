...
int main(int argc, char **argv)
{
  ...
  struct data eta, u, v;
  init_data(&eta, nx, ny, param.dx, param.dy, 0.);  // TODO make sure dx,dy on other files too
  ...
  // TODO should this be parallelized ? nested omp pragma not allowed
  for(int n = 0; n < nt; n++) {
    ...
    if(param.source_type == 1) {
      ...
      double f = 1. / 20.;
      for(int j = 0; j < ny; j++) {
        for(int i = 0; i < nx; i++) {  // TODO: already changed order of these 2 loops
          SET(&u, 0, j, 0.);
          SET(&u, nx, j, 0.);
          SET(&v, i, 0, 0.);
    ...
    // update u and v  // TODO I am sure we can merge these loops with the ones above
    #pragma omp for collapse(2)
    for(int j = 0; j < ny; j++) {
      for(int i = 0; i < nx; i++) {  // TODO: already changed order of these 2 loops
